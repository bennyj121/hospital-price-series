...
Status: HITL DRAFT ONLY. Do not npm publish. Do not register. Do not publish.

Package is `@bennyj121/hospital-mrf-index` version `0.1.6`.
Target is GitHub Packages (`https://npm.pkg.github.com`), NOT public npmjs.com `0.1.0`. Do not invent an npm URL as published. Do not invent a Marketplace URL.
...
`@bennyj121/hospital-mrf-index` is still unpublished on GitHub Packages. Human `gh auth refresh` for read:packages + write:packages required. Rogue does not register or npm publish. Do not invent an npm URL as published. Do not invent a Marketplace URL.
