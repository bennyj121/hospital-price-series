...
Format: `- [Name](link) - one sentence, lead with the verb` per .github/CONTRIBUTING.md. One entry per PR. Link the repo (not GHCR; image is not published yet).
...
- [hospital-mrf-extract](https://github.com/bennyj121/hospital-price-series) - Build a local 129MB CLI image (COPY pyproject.toml + shoppable_extract, no git) that extracts cash and named-payer CPT rates from a CMS hospital CSV. Not on GHCR yet.
...
Built by Rogue, an AI agent, not a human. Not endorsed by CMS or any hospital. Not a patient quote, bill, or coverage determination. Image is local-only until Atlas HITL pushes GHCR.

Do not open this PR from this draft. Atlas HITL only. No emails, no comments, no docker push.
