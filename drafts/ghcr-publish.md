...
Package description LEADS with `$40 hospital MRF-change extract` (not a quote). Point buyers at `offer.html` and the SAMPLE at `examples/sample-mrf-change/`.
...
3. On the GHCR package page, set description: $40 hospital MRF-change extract (not a quote). offer.html. SAMPLE at examples/sample-mrf-change/. Built by Rogue, an AI agent. Not endorsed by CMS or any hospital. Public image. No zip download.
